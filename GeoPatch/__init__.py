...
class Generator:
    ...
    def readData(self):
        ...
        return self.img, self.imgarr, self.inv
    
    def data_dimension(self):
        """
        To display nput data dimension
        """
        self.readData()
        print("############################################")
        print(f' the shape of image is: {self.imgarr.shape} ')
    ...
    def patch_info(self):
        ...
        if self.channel==True:
            ...
            x_dim=self.imgarr.shape[0]
            y_dim=self.imgarr.shape[1]
        ...
        #return self.X, self.Y, self.total_patches
    ...
    def save_Geotif(self, folder_name="tif", only_label=False):
        ...
        only_label:         Boolean
                            If True, only the patches will be exported that have labeled data.
        ...
        with tqdm.tqdm(range(self.total_patches), desc="Patch Counter", unit=" Patch") as pbar:
            ...
            print(" -----------------------------------------------------------------------------------")
            print(f'{pbar.total} patches are saved as ".tif" format in "{os.getcwd()}\{folder_name}"')
            print("#######################################################################################")
    ...
    def save_numpy(self, folder_name="npy", only_label=True, return_stacked=True, save_stack=True,  V_flip=True, H_flip=True, Rotation=True):
        ...
        if only_label==True:
            ...
            print(" --------------------------------------------------------------------------------------")
            print(f"{save_patches} patches which are %{percentage} of {total_patches} total patches had labeled data\n"
                  f'are saved as ".npy" format in "{os.getcwd()}\{folder_name}"')
    ...
    def visualize(self,folder_name="npy",patches_to_show=1, band_num=1, fig_size=(10,20), dpi=96):
        ...
        idx=np.random.randint(1, self.total_patches, patches_to_show)
